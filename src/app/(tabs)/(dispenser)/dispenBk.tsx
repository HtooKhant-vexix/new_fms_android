...
export default function DispenserScreen() {
	...
	// 		`android-${parseInt((Math.random() * 100).toString())}`,
	// 	)
	...
	const parseResponse = (
		...
		const calculatedCRC = calculateCRC(Buffer.from(response?.slice(0, -2)))
	...
	// 	return dispensers.filter((dispenser) => filData?.includes(dispenser?.nozzle_no))
	// }, [dispensers])

	const nozData = useMemo(() => {
		return dispensers.filter(
			(dispenser) => Array.isArray(filData) && filData.includes(dispenser?.nozzle_no),
		)
	}, [dispensers])
					...
					<ScrollView contentContainerStyle={styles.scrollContent}>
						...
						<View style={styles.grid}>
							...
							<Dispenser
								...
								key={nozData[0]?._id}
								noz={parseInt(nozData[0]?.nozzle_no || '0', 10)}
